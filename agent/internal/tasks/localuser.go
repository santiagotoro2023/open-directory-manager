...
func addLocalUser(ctx context.Context, payload map[string]any, env apply.Env) (string, error) {
	...
	if out, err := env.Run.Run(ctx, "useradd", append(arguments, name)...); err != nil {
	...
	if password == "" {
		...
		if out, err := env.Run.Run(ctx, "passwd", "--lock", name); err != nil {
...
func removeLocalUser(ctx context.Context, payload map[string]any, env apply.Env) (string, error) {
	...
	out, err := env.Run.Run(ctx, "userdel", "--remove", name)
	if err != nil {
