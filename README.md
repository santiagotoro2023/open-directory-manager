...
```bash
sudo apt update
sudo DEBIAN_FRONTEND=noninteractive apt install ./odm-client_0.17.1_amd64.deb
...
| Password manager | A role that runs Vaultwarden (the open-source Bitwarden server) on a member server, managed entirely from the console: one press sets it up (the console makes its own account in the vault and an organisation named after the domain), and from then on seats (domain groups and accounts), collections and which groups see them are decided on the Passwords page and reconciled into the vault by the console — invitations, confirmations, groups, collections, access. The vault is at the console's own address, `/vault`, carried to whichever server holds it over TLS the console verifies; people sign in with their domain account through the console — the domain's OpenID Connect provider — with no prompt at all from a domain-joined desktop; the extension and the desktop app arrive on workstations by policy, already pointed at the vault. Each person's master password stays their own (the administrator's can be set to their domain password at setup): it is what encrypts their vault, and nothing on the server can stand in for it |
...
| Password manager | Vaultwarden in a podman quadlet, carried under the console's `/vault` by the control plane (`api/odm/vaultproxy.py`); the control plane as a Bitwarden client of its own (`api/odm/bitwarden.py`, `vaultkeeper.py`) owning the organisation and reconciling seats, groups and collections; and as an OpenID Connect provider (`api/odm/oidc.py`) for sign-in |
...
```bash
bash packaging/deb/build-in-container.sh 0.17.1   # -> dist/odm-client_0.17.1_amd64.deb
```
