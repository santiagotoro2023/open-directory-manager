    ...
    <>
      <Quickstart>
        <p>
          ...
          <C>//server/share</C>. ODM holds the definition; the server&rsquo;s agent renders it
          into the file server&rsquo;s configuration and into the directory&rsquo;s permissions on
          its next check-in.
        </p>
        ...
        <Example title="Publish a directory">
          <Steps>
            ...
            <li>
              <strong>File Shares</strong> → <strong>New share</strong> → pick the server, name
              the share, give the directory it publishes.
            </li>
            <li>
              Add permission entries: a user or group, and one of{" "}
              <strong>Read</strong>, <strong>Read &amp; write</strong> or{" "}
        ...
        <Example title="Give a group access to an existing share">
          Open the share → <strong>Add entry</strong> → <strong>Select…</strong> → choose the
          group → set the access level → <strong>Save</strong>.
        </Example>
        ...
        <Example title="Stop sharing a directory">
          Right-click the share → <strong>Stop sharing</strong>. The directory and its contents
          stay on the server.
        </Example>
      ...
      <Details>
        <Section title="Access levels">
          <p>
            Each level is a fixed set of permissions. <strong>Applies to new files</strong> adds
            the matching default entry, so files created in the share afterwards carry it too.
          </p>
          <Reference
        ...
        <Section title="Where a share ends up on the server">
          ...
          />
          <p>
            The include file is rewritten in full each time, so a share edited by hand on the
            server is replaced by what the console holds. Everything else in{" "}
            <C>smb.conf</C> is left alone.
          </p>
        </Section>

        <Section title="Clients">
          <p>
            Domain members mount shares with <C>sec=krb5</C>, so a workstation uses the
            user&rsquo;s existing Kerberos ticket and no share credential is stored on it. A
            drive-map policy is the usual way to mount one.
          </p>
        </Section>
