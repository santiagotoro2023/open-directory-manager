import { C, Code, Details, Example, Quickstart, Reference, Section, Where } from "../components";
...
export const meta: WikiPageMeta = {
  ...
  title: "Policy agent",
  section: "Clients",
  summary: "The agent on every domain member: how it authenticates, when it runs, and what it reports.",
...
export function Content() {
  return (
    <>
      <Quickstart>
        <p>
          <C>odm-agent</C> runs on every domain member. It authenticates with the machine&rsquo;s
          own Kerberos keytab, asks the control plane what policy applies to it, applies the
      ...
      <Details>
        <Section title="Commands">
          <Reference
            ...
            rows={[
              [<C key="1">odm-agent apply</C>, "Fetches and applies policy if it has changed since the last run."],
              [<C key="2">odm-agent apply --force</C>, "Applies regardless of whether anything changed."],
              [<C key="3">odm-agent apply --user NAME</C>, "Applies the policy resolved for one user on this machine."],
              [<C key="4">odm-agent daemon</C>, "Applies on the refresh interval. This is what the service runs."],
        ...
        <Section title="The refresh cycle">
          <Reference
            ...
            rows={[
              ["Authenticate", "SPNEGO with the machine keytab. No agent credential exists separately."],
              ["Report facts", "The operating system identifier and current addresses, for item-level targeting."],
              ["Compare", "If the policy fingerprint matches the last applied one, nothing is done."],
          ...
          />
          <p>
            The interval defaults to 15 minutes and is itself policy. A small random offset is
            added so a fleet does not check in at the same instant.
          </p>
        </Section>
        ...
        <Section title="Files the agent owns">
          <Reference
            ...
            rows={[
              ...
              [<C key="3">/var/lib/odm/last-serial</C>, "The fingerprint of the last applied policy."],
